...
import { useMoodHistory, MoodHistoryEntry } from '@/hooks/use-mood-history'; // Importez le nouveau hook
...
// --- Composants ---
const Icon = ({ path, size = 24, color = theme.colors.foregroundLight }: { path: string, size?: number, color?: string }) => (
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <View style={styles.headerBottom}>
          <Text style={styles.headerSubtitle}>Suivez lՎvolution de votre humeur</Text>
        </View>
        <View>
                  ...
                  <View style={styles.timelineContainer}>
                    {timelineData.map(item => (
                      <View key={item.id} style={styles.timelineItem}>
                        <Text style={styles.timelineEmoji}>{moodValueToEmoji(item.mood_value)}</Text>
                        <View style={styles.timelineContent}>
                          <View style={styles.timelineHeader}>
                            <Text style={styles.timelineMood}>{item.mood_label}</Text>
                            <Text style={styles.timelineDate}>
                                {format(new Date(item.logged_at), 'd MMM', { locale: fr })}
                            </Text>
                          </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 18, fontWeight: '700', color: theme.colors.foregroundLight },
  headerBottom: { paddingHorizontal: 16, paddingVertical: 16 },
  headerSubtitle: { fontSize: 24, fontWeight: '700', color: theme.colors.foregroundLight },
